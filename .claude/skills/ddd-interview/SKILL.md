...
Capture what the user wants to build, in their own words, as `specs/Brainstorm/interview.md`. This is exploratory and free-form. It is explicitly **not** the place to produce structured functional requirements (that is `/ddd-map-requirements`) or to name Bounded Contexts (that is `/ddd-map-contexts`, gated by `DDD.md` section 4 — the LLM is forbidden from inferring Bounded Contexts here).
...
2. Ask open-ended questions covering, at minimum:
   ...
   There is no cap on clarifying questions here (unlike `/ddd-map-requirements`'s max-3 rule) — this is meant to be a real conversation. Don't rush to a single round of questions and stop; follow up on interesting or ambiguous answers.

3. If the user starts describing formal requirements ("it must support X req/s", "it must validate emails") or names Bounded Contexts unprompted, capture what they said in this file anyway (it's still useful raw material) but note in your response that formal requirements mapping and context mapping happen in `/ddd-map-requirements` and `/ddd-map-contexts` respectively — don't silently promote their words into those artifacts yourself.
...
- Suggested next step: `/ddd-map-requirements` (if not yet run) to turn this into testable requirements, or `/ddd-interview` again later to add more ground.
