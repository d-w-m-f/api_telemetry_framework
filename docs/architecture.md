...
- The **frontend** (Angular) talks to the **main backend** (Spring Boot) in a standard client-server model,
  and displays data served by it.
- The main backend connects to the **main database** (PostgreSQL).
- The frontend generates telemetry-test payloads, which the main backend enqueues onto a **RabbitMQ**
...
row is written at enqueue time with a serial id, the JSON event, and a status: a boolean `is_completed`
flag that is only flipped on completion (`false` covers everything else — waiting, DLQ, retry, processing,
etc.).

The main backend mints a UUID for each telemetry test at enqueue time and writes it, alongside the payload,
into a `telemetry_runs` row (`is_completed = false`). That same UUID is carried unchanged through the whole
pipeline — the AMQP message body, the sandbox env vars, the `telemetry_results` row — which is what makes
`is_completed` a valid guard against RabbitMQ redelivering the same message twice: a second delivery
references the same row and can see the flag already flipped.
...
- On success, `load_n_telemetry` (inside the sandbox) writes the finished results directly into that
  main-DB row itself — the consumer doesn't need to relay results on the happy path, it just flips
